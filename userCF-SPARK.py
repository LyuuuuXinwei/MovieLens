...
if __name__ == "__main__":
    ...
    '''
    item_user_pairs = lines.map(parseVectorOnItem).groupByKey().map(lambda x: sampleInteractions(x[0], x[1], 500)).cache()
    ...
    pairwise_users = item_user_pairs.filter(
        ...
        lambda x: findUserPairs(x[0], x[1])).groupByKey()
    ...
    user_sims = pairwise_users.map(
        ...
        lambda x: nearestNeighbors(x[0], x[1], 50))
    ...
    user_item_recs = user_sims.map(
        lambda x: topNRecommendations(x[0], x[1], uib.value, 100)).collect()
