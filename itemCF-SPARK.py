...
if __name__ == "__main__":
    ...
    user_item_pairs = lines.map(parseVector).groupByKey().map(
        lambda x: sampleInteractions(x[0], x[1], 500)).cache()
    ...
    pairwise_items = user_item_pairs.filter(
        ...
        lambda x: findItemPairs(x[0], x[1])).groupByKey()
    ...
    item_sims = pairwise_items.map(
        ...
        lambda x: nearestNeighbors(x[0], x[1], 50)).collect()
    ...
    user_item_recs = user_item_pairs.map(
        lambda x: topNRecommendations(x[0], x[1], isb.value, 500)).collect()